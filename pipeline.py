...
{partials_json}"""
                ...
                try:
                    ...
                    try:
                        ...
                        print("Reduce phase JSON parsing failed, using manual merge")
                    ...
                    print(f"Reduce phase error: {e}, using manual merge")
                    # Fallback: merge partials manually
                    questions = {"mcq": [], "short": []}
                    for partial in all_partials:
                        questions["mcq"].extend(partial.get("mcq", []))
                        questions["short"].extend(partial.get("short", []))
                    questions = validate_and_normalize_questions({"questions": questions}, max_mcq, max_short)
                    print(f"DEBUG: Manual merge after error: {len(questions.get('mcq', []))} MCQ, {len(questions.get('short', []))} short")
            else:
                print("DEBUG: No partial results to reduce")
                questions = {"mcq": [], "short": []}
